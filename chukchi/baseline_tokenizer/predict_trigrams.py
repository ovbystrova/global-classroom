...
(unigrams, bigrams, trigrams) = pickle.load(mf, encoding='utf-8')
        ...
        if first in trigrams:
            if second in trigrams[first]:
                ...
                if pred == third:
                    ...
                    output += [c for c in tokenizer.decode([third])]
        # If we haven't found a trigram, we just try proposing the most frequent unigram
        else:
